...
module.exports = {
  ...
  category:'Ticket',
  run: async(client, message, args) => {
    let ticketChannel = message.guild.channels.cache.find(channel => channel.name.toLowerCase() === 'ticket-box')
    ...
    // }
    if(!ticketChannel){
      return message.reply("You haven't setuped the ticket system, do $setup")
    }
    if(ticketChannel.id != message.channel.id)
    {
