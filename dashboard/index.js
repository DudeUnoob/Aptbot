...
module.exports = client => {
  ...
  //     let custom;
  //     let px = await prefixSchema.findOne({ Guild : guild.id })
  ...
  //     if(req.body.prefix) prefixSchema.findOne({ Guild: guild.id }), async(err, data) => {
  //       if(err) throw err;
  //       if(data) {
  ...
  //     }
      ...
      let pfx = prefixSchema.findOne({ Guild : guild.id }, async(err, data) => {
        if(err) throw err;
        if(data) {
            await prefixSchema.findOneAndDelete({ Guild : guild.id })
            data = new prefixSchema({
      ...
      pfx
        ...
        // let call = await prefixSchema.findOne({ Guild : guild.id })
        // .then(custom => {
        //   custom.Prefix
        //   //console.log(custom)
        // })
        
      // let callPrfx = await prefixSchema.findOne({ Guild : guild.id })
      // let custom = callPrfx?.Prefix
      //let custom = callPrfx.Prefix;
